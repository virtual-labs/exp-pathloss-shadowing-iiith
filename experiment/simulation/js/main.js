...
// Corrected Okumura Model path loss calculation
function calculatePathLoss(G, distance, frequency, ht, hr) {
    const c = 3 * 1e8; // Speed of light
    const lamda = c / frequency; // Wavelength in meters
    
    // 1. Calculate Free-Space Path Loss L(fc, d)
    // Assuming ht, hr, and d are in meters and fc is in Hz
    let L_fcd;
    const dc = (4 * ht * hr) / lamda; // Crossover distance
    
    if (distance < dc) {
        // Free-space formula (d < dc)
        L_fcd = 20 * Math.log10((4 * Math.PI * distance) / lamda);
    } else {
        // Two-ray model approximation (d >= dc)
        L_fcd = 40 * Math.log10(distance) - 10 * Math.log10(ht * ht * hr * hr);
    }

    // 2. Calculate Transmitter Antenna Height Gain G(ht) based on Okumura model
    let G_ht;
    if (ht > 30 && ht < 1000) {
        G_ht = 20 * Math.log10(ht / 200);
    } else if (ht <= 30) {
        G_ht = 0; // Or a more appropriate value, as formula is for ht > 30
    } else {
        G_ht = 0; // For ht >= 1000
    }
    
    // 3. Calculate Receiver Antenna Height Gain G(hr) based on Okumura model
    let G_hr;
    if (hr <= 3) {
...
    } else {
        // For hr >= 10, use the last formula from the given range.
        G_hr = 20 * Math.log10(hr / 3);
    }
    
    // 4. Combine terms for total path loss (ignoring A_mu and G_AREA)
